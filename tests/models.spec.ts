...
describe('models helper', () => {
  ...
  it('fetchModelsCached caches results by ttl', async () => {
    ...
    const second = await fetchModelsCached({ domain: 'https://openrouter.ai/api/v1', ttlMs: 60000 });
  ...
  it('debouncedSuggestFactory falls back to default when fetch fails', async () => {
    ...
    (listModels as any).mockImplementationOnce(async () => { throw new Error('offline'); });
    const suggest = debouncedSuggestFactory({ domain: 'https://openrouter.ai/api/v1', debounceMs: 200 });
    const p = suggest('ll');
